...
for k in range(ind):
    ...
    sottofinestra_unwrap=unwrap.sel(x=slice(windxmin,windxmax),y=slice(windymin,windymax))
    ...
    sottofinestra_dem=dem.sel(x=slice(windxmin,windxmax),y=slice(windymin,windymax))
    #faccio lo stack per ottenere la coppia di coordinate per ogni valore della z
    ...
    regressione= Polynomial.fit(z_unwrap[no_nan==False], z_dem[no_nan==False], deg=1).convert()
